...
```
  ...
  This RMarkdown file contains analysis conducted on BRFSS survey data from 2012-2019 in regards to identifying causal changes in health outcomes due to implementation of reinsurance programs. This analysis operates on the basis of a difference-in-differences research design, with treatment implementation occurring at two separate times, one group with treatment implemented in 2018 and another with treatment in 2019. The difference between this file and "Primary Research Analysis.Rmd" is that the analysis conducted in this file omits Wisconsin and the other control states which did not adopt Medicaid expansions-- the results here then strictly apply to states which did have Medicaid expansion policies. Variables used in this analysis are described below:
...
# Subset for only medicaid expansion states, meaning that we
# drop Wisconsin, Wyoming, South Dakota and Kansas.
complete <- subset(complete, STATE != 55 & STATE != 56 &
                     STATE != 46 & STATE!= 20)
...
write.matrix.to.txt(genhlth.results,
                    "genhlth_expansion_results.txt")
write.matrix.to.txt(checkup.results,
                    "checkup_expansion_results.txt")
write.matrix.to.txt(ovrhlth.results,
                    "ovrhlth_expansion_results.txt")
write.matrix.to.txt(phys.results,
                    "phys_distress_expansion_results.txt")
write.matrix.to.txt(ment.results,
                    "ment_distress_expansion_results.txt")
write.matrix.to.txt(hlthpln.results,
                    "hlthpln_expansion_results.txt")
write.matrix.to.txt(medcost.results,
                    "medcost_expansion_results.txt")
write.matrix.to.txt(persdoc.results,
                    "persdoc_expansion_results.txt")
```
